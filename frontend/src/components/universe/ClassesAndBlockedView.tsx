...
import { Shield, Ban, Clock, Save, Loader2, Activity } from 'lucide-react';
...
export const ClassesAndBlockedView: React.FC<ViewProps> = ({ config, snapshot, onConfigUpdate }) => {
    ...
    const toggleClass = async (cls: string, enabled: boolean) => {
        await api.post(`/api/universe/class/toggle?asset_class=${cls}&enabled=${enabled}`, {});
        onConfigUpdate();
    };
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        {config.classes_enabled && Object.entries(config.classes_enabled).map(([cls, enabled]) => {
                            const count = snapshot?.class_counts?.[cls] ?? 0;
                            return (
                                <div key={cls} className={`p-4 rounded-lg border flex items-center justify-between ${enabled ? 'bg-card border-border' : 'bg-muted/30 border-muted opacity-80'}`}>
                ...
                <div className="space-y-4">
                    ...
                    <div className="flex gap-2">
                        ...
                        <button
                            onClick={addBlock}
                            className="bg-destructive/10 text-destructive hover:bg-destructive/20 px-4 py-2 rounded text-sm font-semibold"
                        >
                            Block
                        </button>
                    </div>
                    ...
                    <div className="space-y-2 max-h-[300px] overflow-auto">
                        ...
                        {config.blocklist && config.blocklist.map(s => (
                            <div key={s} className="flex items-center justify-between p-2 rounded bg-muted/20 border border-border">
